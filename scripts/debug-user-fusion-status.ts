...
async function debugUserFusionStatus() {
  try {
    ...
    if (!user) {
      console.log('❌ User not found in LocalBoxs database');
      console.log('💡 You may need to create a user account first');
      return;
    }

    console.log('✅ User found in LocalBoxs database:');
    ...
    if (!user.fusionSubAccountId) {
      ...
      if (existingFusionUser) {
        ...
        // Update LocalBoxs user with Fusion sub-account ID
        console.log('🔗 Linking LocalBoxs user to Fusion sub-account...');
