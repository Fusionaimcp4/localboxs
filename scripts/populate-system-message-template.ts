  ...
  try {
    ...
    if (existingTemplate) {
      ...
      if (!shouldOverwrite) {
        ...
        console.log('   Example: npm run populate-template -- --force');
        return;
      }
    ...
    if (!fileContent) {
      ...
      console.error('   Make sure the file exists and contains content.');
      return;
    }
...
if (args.includes('--help') || args.includes('-h')) {
  console.log(`
📋 SystemMessageTemplate Population Script

Usage:
  npm run populate-template              # Check if template exists
  npm run populate-template -- --force   # Force update existing template
...
Examples:
  npm run populate-template
  npm run populate-template -- --force
  npx tsx scripts/populate-system-message-template.ts --force
`);
  process.exit(0);
}
